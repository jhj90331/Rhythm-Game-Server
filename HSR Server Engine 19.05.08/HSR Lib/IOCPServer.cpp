...
DWORD WINAPI IOCPServer::workerThread(LPVOID serverPtr)
{
	...
	while (!_shutdown)
	{
		...
		try {
			...
			//SLog(L"GQCS Thread : %d", std::this_thread::get_id());
			if (!ret)
			{
				throw session;
//				continue;
			...
			if (transferSize == 0)
			{
				//SLog(L"GQCS Error : %d", WSAGetLastError());
				SLog(L"*transferSize close by client[%d][%s]", session->id(), session->clientAddress().c_str());
			...
			{
			case IO_WRITE:
				session->onSend((size_t)transferSize);	// HACK : 가변길이 패킷  
				continue;
			...
			//		SLog(L"Session packetQueue size : %d \n", session->packageQueue.size());
