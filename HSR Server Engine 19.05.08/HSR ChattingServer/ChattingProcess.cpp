...
#define INSERT_PACKET_PROCESS(type)		runFuncTable_.insert(make_pair(E_##type, &ChattingProcess::##type))
	...
	INSERT_PACKET_PROCESS(C_REQ_MOVING);
}
...
void ChattingProcess::I_CHTTING_NOTIFY_ID(Session *session, Packet *rowPacket)
{
	...
	SLog(L"[%d] : clientId_ [%d] : oidAccoundId", packet->clientId_, packet->oidAccountId_);
...
void ChattingProcess::C_REQ_CHATTING(Session *session, Packet *rowPacket)
{
	...
	// 주석부분은 에코
	for (auto iter = SESSIONMANAGER.sessionList().begin(); iter != SESSIONMANAGER.sessionList().end(); iter++)
	{
		if ((*iter)->type() == SESSION_TYPE_CLIENT)	// 세션 타입을 검사해서 터미널이아닌 클라이언트이면 채팅 패킷을 보내줌
		{
			//if (*iter != session)
			//{
			(*iter)->sendPacket(&retPacket);
			//}
		}
	}
...
void ChattingProcess::C_REQ_MOVING(Session *session, Packet *rowPacket)
{
	PK_C_REQ_MOVING *packet = (PK_C_REQ_MOVING *)rowPacket;

	PK_S_ANS_MOVING movPacket;

	movPacket.posX_ = packet->posX_ + 0.5f;
	movPacket.posY_ = 0;//packet->posY_;
	movPacket.posZ_ = 0;//packet->posZ_;

	session->sendPacket(&movPacket);
}